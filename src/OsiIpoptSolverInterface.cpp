...
// used in getting iteration count
#include "IpSolveStatistics.hpp"

#include "OsiIpoptSolverInterface.hpp"

// TODO(aykut):
// 1. Implement initialSolve, done.
...
void OsiIpoptSolverInterface::initialSolve() {
  if (IsNull(ipoptProb_)) {
    // this might happen if problem is loaded using loadProblem and ipoptProb_
    // is not allocated yet.
    ipoptProb_ = new OsiIpoptTNLP(matrix_, rev_matrix_, rowlb_, rowub_,
                                  collb_, colub_, obj_, numCones_,
                                  coneSize_, coneType_, coneMembers_);
  }
  app_->Options()->SetIntegerValue("print_level", printLevel_);
  status_ = app_->OptimizeTNLP(ipoptProb_);
}
...
void OsiIpoptSolverInterface::resolve() {
  if (IsNull(ipoptProb_)) {
    ...
    app_->Options()->SetIntegerValue("print_level", printLevel_);
    status_ = app_->ReOptimizeTNLP(ipoptProb_);
  }
}
...
const double * OsiIpoptSolverInterface::getColSolution() const {
  OsiIpoptTNLP const * prob = dynamic_cast<OsiIpoptTNLP *> (GetRawPtr (ipoptProb_));
...
double OsiIpoptSolverInterface::getObjValue() const {
  ...
  double val = 1e+30;
  OsiIpoptTNLP const * prob = dynamic_cast<OsiIpoptTNLP *> (GetRawPtr (ipoptProb_));
  double const * solution = prob->solution();
  if (solution) {
    val = std::inner_product(obj_, obj_+n, solution, 0.0);
  }
  return val;
}
